...
class PaymentTransaction(models.Model):
    ...
    def _paymongo_prepare_checkout_session_payload(self):
        ...
        # Use web.base.url (tunnel) instead of get_base_url() which may
        # return the LAN IP the mobile app connects from.
        icp = self.env['ir.config_parameter'].sudo()
        base_url = icp.get_param('web.base.url', self.provider_id.get_base_url())
        return_url = urljoin(base_url, PayMongoController._return_url)

        # Odoo access token for "return" route safety (like Xendit pattern)
        access_token = payment_utils.generate_access_token(self.reference, self.amount, env=self.env)
    ...
    @staticmethod
    def _paymongo_webhook_event(payment_data):
        """Return (event_type, livemode, resource, resource_attrs) from a webhook payload.

        Real PayMongo webhook envelope (verified 2026-09-11 from a live event):
          data.type                -> 'event' (envelope marker, do not use as event type)
          data.attributes.type     -> real event type, e.g. payment.paid
          data.attributes.livemode -> test / live mode flag
          data.attributes.data     -> the resource object (payment/checkout_session)
          data.attributes.data.attributes -> the resource attributes
        Older docs samples used the shorter shape:
          data.type                -> event type
          data.livemode            -> mode flag
          data.data.attributes     -> the resource attributes
        Returns the first (event_type, livemode, resource, resource_attrs) that exists.
        """
        event = (payment_data or {}).get('data') or {}
        attrs = event.get('attributes') or {}

        # Prefer the real envelope (event type + resource live under attributes).
        if attrs.get('type'):
            resource = attrs.get('data') or {}
            resource_attrs = resource.get('attributes') or {}
            return attrs.get('type'), attrs.get('livemode'), resource, resource_attrs

        # Fallback to the documented shorter shape.
        resource = event.get('data') or {}
        resource_attrs = resource.get('attributes') or {}
        return event.get('type'), event.get('livemode'), resource, resource_attrs
    ...
    def _extract_reference(cls, provider_code, payment_data):
        ...
        _, _, _, resource_attrs = cls._paymongo_webhook_event(payment_data)
    ...
    def _apply_updates(self, payment_data):
        ...
        event_type, _, session, session_attrs = self._paymongo_webhook_event(payment_data)
        ...
        if event_type in ("checkout_session.payment.paid", "payment.paid"):
            self._set_done()
            # Auto-confirm linked sale orders so wallet credits are applied.
            for so in self.sale_order_ids:
    ...
    def _extract_amount_data(self, payment_data):
        ...
        _, _, _, session_attrs = self._paymongo_webhook_event(payment_data)
        if not isinstance(session_attrs, dict):
            session_attrs = {}

        # 1) Preferred: payment.attributes.amount (real payment.paid resource)
        amount_minor = session_attrs.get('amount')
        if amount_minor is not None:
            return {
                "amount": self._paymongo_from_minor_currency_units(int(amount_minor)),
                "currency_code": session_attrs.get('currency') or self.currency_id.name,
                "precision_digits": self.currency_id.decimal_places,
            }

        # 2) checkout_session.attributes.payments[0].attributes.amount
        ...
        # 3) Fallback: payment_intent.attributes.amount
